...
public class ApiManager {
    ...
    public interface KemikalijaUpdateCallback {
        void onSuccess();
        void onFailure(Call<Void> call, Throwable throwable);
    }

    public void updateKemikalija(Kemikalija kemikalija, KemikalijaUpdateCallback callback) {
        Call<Void> call = apiService.updateKemikalija(kemikalija);
        call.enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    Log.d("KemikalijaManager", "Kemikalija updated successfully");
                    callback.onSuccess();
                } else {
                    Log.e("KemikalijaManager", "Failed to update Kemikalija: " + response.message());
                    callback.onFailure(call, new Throwable(response.message()));
                }
            }
            ...
            @Override
            public void onFailure(Call<Void> call, Throwable t) {
                Log.e("KemikalijaManager", "Error: " + t.getMessage());
                callback.onFailure(call, t);
            }
        });
